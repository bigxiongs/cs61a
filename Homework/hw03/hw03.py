...
def div_interval(x, y):
    ...
    reciprocal of y."""
    assert not (lower_bound(y) <= 0 <= upper_bound(y)), 'Divided by zero'
...
def check_par():
    ...
    True
    """
    r1 = interval(4, 8) # Replace this line!
...
def quadratic(x, a, b, c):
    ...
    d = lower_bound(x)
    e = upper_bound(x)
    extremum = (4*a*c - b*b) / (4*a)
    if symmetry < d:
        if a > 0:
            return interval(f(d), f(e))
        else:
            return interval(f(e), f(d))
    elif symmetry > e:
        if a > 0:
            return interval(f(e), f(d))
        else:
        ...
        if a > 0:
            return interval(extremum, max(f(e), f(d)))
        else:
            return interval(min(f(e), f(d)), extremum)
