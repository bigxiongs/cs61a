...
def num_splits(s, d):
    ...
    12
    """
    def split_to_two(lst):
        l1 = []
        l2 = []
        for pattern in product([True, False], repeat=len(lst)):
            l1.append([x[1] for x in zip(pattern, lst) if x[0]])
            l2.append([x[1] for x in zip(pattern, lst) if not x[0]])
        yield from zip(l1, l2)

    def check(lst):
        return abs(sum(lst[0]) - sum(lst[1])) <= d

    count_lst = 0
    for pairs in split_to_two(s):
        if check(pairs):
            count_lst += 1
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check):
        if self.holder == check.reciver and not check.deposited:
...
class Check(object):
    def __init__(self, reciver, amount, deposited=False):
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            h = helper_align(skeleton_idx+1, code_idx+1)
            edits = skel_char + h[0]
            cost = 0 + h[1]
            possibilities.append((edits, cost))
        # Insert
        h = helper_align(skeleton_idx, code_idx+1)
        edits = '+[' + code_char + ']' + h[0]
        cost = 1 + h[1]
        possibilities.append((edits, cost))
        # Delete
        h = helper_align(skeleton_idx+1, code_idx)
        edits = '-[' + skel_char + ']' + h[0]
        cost = 1 + h[1]
        possibilities.append((edits, cost))
...
def reverse(lst):
    ...
    True
    """
    return foldl(lst, lambda lst, el: Link(el, lst), Link.empty)
...
def foldl2(link, fn, z):
    ...
    """
    def step(func, num):
        return lambda x: fn(func(x), num)
    return foldl(link, step, identity)(z)
