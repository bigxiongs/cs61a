...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a = distance(destination, city_a)
    dis_b = distance(destination, city_b)
    if dis_a < dis_b:
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        return tree(label(t1) + label(t2), [add_trees(x, y) for x, y in zip_longest(branches(t1), branches(t2), fillvalue=tree(0))])
