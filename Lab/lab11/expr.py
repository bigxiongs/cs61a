...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        operands = [subexpr.eval(env) for subexpr in self.operands]
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        copy_of_parent = self.parent.copy()
        for parameter, argument in zip(self.parameters, arguments):
            copy_of_parent[parameter] = argument
        return self.body.eval(copy_of_parent)
