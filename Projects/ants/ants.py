...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        find_for_bee = self.place
        place_count = 0
        while find_for_bee != beehive and place_count != self.min_range:
            find_for_bee = find_for_bee.entrance
            place_count += 1
        while find_for_bee != beehive and place_count <= self.max_range:
            if find_for_bee.bees != []:
                return rANTdom_else_none(find_for_bee.bees)
            else:
                find_for_bee = find_for_bee.entrance
                place_count += 1
        return None
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        for i in reversed(range(len(self.place.bees))):
            self.place.bees[i].reduce_armor(amount if amount < self.armor else amount + self.damage)
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower, which is a subclass of ThrowerAnt that is more costly and watersafe,
...
# END Problem EC
    ...
    def action(self, gamestate):
        ...
        if self.true_queen:
            ...
            double_ants_place = self.place
            while double_ants_place.exit:
                double_ants_place = double_ants_place.exit
                if double_ants_place.ant and not double_ants_place.ant.doubled:
                    double_ants_place.ant.damage *= 2
                    double_ants_place.ant.doubled = True
        else:
    ...
    def remove_from(self, place):
        """The true queen cannot be removed. Attempts to remove the queen
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return True if self.place.ant is not None and self.place.ant.blocks_path else False
    ...
    def action(self, gamestate):
        ...
        # BEGIN EC
        if self.direction == -1:
            if self.place.entrance is Hive:
...
def make_scare(action, bee):
    ...
    # BEGIN Problem Optional 4
    def new_action(gamestate):
        if bee.is_scared_before is False:
            bee.direction *= -1
