...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operator = expr.first
        operands = expr.rest
        operator = scheme_eval(operator, env)
        def scheme_eval_map(operand):
            return scheme_eval(operand, env, _)
        operands = operands.map(scheme_eval_map)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        param = formals
        v = vals
        while param is not nil and v is not nil:
            child_frame.bindings[param.first] = v.first
            param = param.rest
            v = v.rest
        return child_frame
        # END PROBLEM 10
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        if scheme_procedurep(expressions.rest):
            env.bindings[target] = scheme_eval(expressions.rest, env)
        else:
        ...
        formals = target.rest
        target = target.first
        expressions = Pair(formals, expressions.rest)
        env.bindings[target] = do_lambda_form(expressions, env)
        return target
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    def f(bindings):
        ...
        names = Pair(name, nil)
        vals = Pair(value, nil)
        names.rest, vals.rest = f(bindings.rest)
        return names, vals
    names, values = f(bindings)
